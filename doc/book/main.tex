...
The command requesting user input always pushes a new bubble inside
the abyss, thus an empty input is represented by a double bubble
containing a single end of line marker; we push inside the abyss an
\enquote{empty string} double bubble and compare the top bubble with
the one below.
...
operating on each contained bubble.
...
compare the double bubble we created with the input taken from the
user: if the input is not empty execution will reach the
\verb|~wa awa awa awawa| command, jumping back to the beginning. If
the two bubbles are the same, however, the \verb|~wawawawawa| command
will be executed, terminating the program immediately.
